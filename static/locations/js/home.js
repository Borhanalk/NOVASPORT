    ...
    locations.forEach((location) => {
        ...
        marker.bindPopup(`
            <div class="popup-content">
                <h3>${location.name}</h3>
                <p>Type: ${location.field_type}</p>
                <p>${location.description}</p>
                ${location.image ? `<img src="${location.image}" alt="${location.name}" style="max-width: 100px; height: auto;">` : ""}
                <p><strong>Address:</strong> ${location.address}</p>
                <a href="/locations/${location.id}/" target="_blank" style="color: blue; text-decoration: underline;">View Details</a>
            </div>
        `);
    });
}
